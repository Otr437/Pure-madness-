...
setInterval(() => {
  ...
  redis.set('admin:metrics', JSON.stringify(metrics), 'EX', 60).catch(() => {});
}, 10000);
...
function requireAdminKey(req, res, next) {
  ...
  metrics.errors++;
  res.status(403).json(buildErrorResponse(ERROR_CODES.ADMIN_ACCESS_REQUIRED));
}
...
app.get('/dashboard', async (req, res) => {
  try {
    ...
    const [authHealth, walletHealth, gameHealth, monitorHealth] = await Promise.allSettled([
      axios.get(`${SERVICES.AUTH}/health`, { timeout: 2000 }).then(r => r.data),
      axios.get(`${SERVICES.WALLET}/health`, { timeout: 2000 }).then(r => r.data),
      axios.get(`${SERVICES.GAME}/health`, { timeout: 2000 }).then(r => r.data),
      axios.get(`${SERVICES.MONITOR}/health`, { timeout: 2000 }).then(r => r.data)
    ]);
...
app.put('/users/:playerId/verify', async (req, res) => {
  try {
    ...
    console.log(`✅ Admin verified user: ${playerId}`);
...
app.put('/users/:playerId/balance', async (req, res) => {
  ...
  try {
    ...
    metrics.adminActions++;
    console.log(`✅ Admin adjusted balance for ${playerId}: ${amount} ZEC (${reason})`);
...
app.delete('/games/:gameId', async (req, res) => {
  ...
  try {
    ...
    metrics.adminActions++;
    console.log(`✅ Admin cancelled game ${gameId}: ${reason || 'No reason provided'}`);
...
app.post('/withdrawals/:withdrawalId/cancel', async (req, res) => {
  ...
  try {
    ...
    metrics.adminActions++;
    console.log(`✅ Admin cancelled withdrawal #${withdrawalId}: ${reason || 'No reason provided'}`);
...
app.get('/monitoring/stats', async (req, res) => {
  try {
    const response = await axios.get(`${SERVICES.MONITOR}/stats`, { timeout: 5000 });
...
app.post('/monitoring/trigger-deposits', async (req, res) => {
  try {
    const response = await axios.post(`${SERVICES.MONITOR}/trigger-deposits`, {}, { timeout: 5000 });
    metrics.adminActions++;
    res.json(response.data);
  } catch (error) {
    metrics.errors++;
    console.error('Trigger deposits error:', error);
...
app.post('/monitoring/trigger-withdrawals', async (req, res) => {
  try {
    const response = await axios.post(`${SERVICES.MONITOR}/trigger-withdrawals`, {}, { timeout: 5000 });
    metrics.adminActions++;
    res.json(response.data);
  } catch (error) {
    metrics.errors++;
    console.error('Trigger withdrawals error:', error);
